		...
		for url in urls: 
			...
			if html: 
				...
				for tr in trs: 
					...
					#print(response_time)
					proxy = { "scheme": scheme, "ip": ip, "port": port, "status": verify_result, "response_time": response_time, }
					self.mysql.add_proxy(proxy) 
					'''
					print(scheme,ip,port)
					verify_result = self.verify.verify_proxy(scheme, ip, port) 
					if verify_result["status"] == '1': 
						proxy = { "scheme": scheme, "ip": ip, "port": port, "status": verify_result["status"], "response_time": verify_result["response_time"], } 
						# 存入数据库 
						self.mysql.add_proxy(proxy) 
